...
function computeAchievements(entries, discussions, journal, reflections){
  const streak = calcStreak(entries); const total = entries.length; const a=[];
  ...
  if (negCount && entries.length>=5) a.push({code:'resilience', name:'Resilience'});
  return a;
}
...
export function AppProvider({children}){
  ...
  const [pairedData, setPairedData] = useLocalStorage('pairedData', null);
  ...
  const addEntry = (youInput)=>{
    ...
    setEntries(prev => { const filtered = prev.filter(e=>e.date!==date); return [...filtered, newEntry].sort((a,b)=>a.date.localeCompare(b.date)); });
  };
